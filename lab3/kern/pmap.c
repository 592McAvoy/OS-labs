...
mem_init(void)
{
	...
	uint32_t PISIZE = sizeof(struct PageInfo);
...
page_init(void)
{
	...
	//     never be allocated.
	//  4) The	n extended memory [EXTPHYSMEM, ...).
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!pt_info)
		return NULL;
	pt_info->pp_ref += 1;
	physaddr_t pde_addr =  page2pa(pt_info); 
	pgdir[pd_idx] = pde_addr | PTE_P | PTE_W | PTE_U;
	pte_t* pgtbl = KADDR(PTE_ADDR(pde_addr));
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	uint32_t num = (ROUNDUP(size+va, PTSIZE) - va) / PTSIZE;
	//cprintf("%d large pages\n",num);
	for(int i=0;i<num;i++){
		uint32_t pd_idx = PDX(va+i*PTSIZE);
		physaddr_t addr = pa+i*PTSIZE;
		pgdir[pd_idx] = addr | PGOFF(perm | PTE_P | PTE_PS);
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	assert(pgdir);
	pte_t* pte;
	struct PageInfo* info = page_lookup(pgdir, va, &pte);
	if(!info)
		return;
	page_decref(info);
	memset(pte, 0, sizeof(pte_t));
	tlb_invalidate(pgdir, pte);
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(int i=0;i<n;i++){
		...
		va_ptr = ROUNDDOWN(va_ptr+PGSIZE, PGSIZE);
	}*/
